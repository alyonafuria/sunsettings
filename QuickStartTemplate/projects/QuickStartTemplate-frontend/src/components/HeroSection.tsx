...
RandomSeed: ${seed}`.trim()
    ...
    try {
      ...
      const norm = (p: any) => typeof p === 'number' ? Math.min(100, Math.max(0, Math.round(p))) : null
      let prob = norm(parsed.probability)

      const prev = previousResultRef.current
      const needsRetry = prob === 75 || (prev && prev.location === loc && prev.probability === prob)

      if (needsRetry) {
        try {
          const seed2 = Math.random().toString(36).slice(2, 10)
          const retryParsed = await callModel(seed2)
          const retryProb = norm(retryParsed.probability)
