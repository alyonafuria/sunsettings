    ...
    layers.forEach((layer) => {
        ...
        try {
            ...
            saturationProps.forEach((prop) => {
                try {
                    map.setPaintProperty(id, prop as any, -1)
                } catch {
                    // Ignore errors when setting saturation
                }
            })
            ...
            adjustments.forEach((prop) => {
                try {
                    map.setPaintProperty(id, prop as any, 0)
                } catch {
                    // Ignore errors when setting brightness adjustments
                }
            })
        } catch {
