...
export default function MapCanvas({
  ...
  // Capture initial props for one-time map init to satisfy exhaustive-deps
  ...
  // Map initialization runs once on mount. Subsequent prop changes are handled in a separate effect.
  React.useEffect(() => {
    let cancelled = false
  ...
  React.useEffect(() => {
    ...
    const init = async () => {
      ...
      const initialZoom = initialZoomRef.current ?? 11
      const map = new mapboxgl.Map({
        container,
        style,
        center: initialCenter ? [initialCenter.lon, initialCenter.lat] : [13.404954, 52.520008], // Berlin default
        zoom: initialZoom,
